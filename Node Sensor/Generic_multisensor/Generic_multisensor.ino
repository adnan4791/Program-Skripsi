  ...
  if (millis() > nextTransmissionTime) {
      ...
      for (int i=0; i<number_of_sensors; i++) {
          ...
          if (sensor_ptrs[i]->get_is_connected() || sensor_ptrs[i]->has_fake_data()) {
              ...
              if (i==0) {
                  sprintf(final_str, "%s%s/%s \n", final_str, sensor_ptrs[i]->get_nomenclature(), float_str);
              } 
              else {
                  sprintf(final_str, "%s/%s/%s \n", final_str, sensor_ptrs[i]->get_nomenclature(), float_str);
              }
...
//        PRINT_CSTSTR("%s","Could not switch LoRa module in sleep mode\n");
      ...
      timer.setTimer(LOW_POWER_PERIOD*1);
#endif    
...
//          delay(1);                        
      ...
      //nextTransmissionTime=millis()+(unsigned long)idlePeriodInMin*1*1000+(unsigned long)random(1,1)*1000;
      PRINT_VALUE("%ld", nextTransmissionTime);
      PRINTLN;
  }
#endif
